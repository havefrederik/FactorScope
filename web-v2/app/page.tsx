...
type DemoKey = "equity" | "market-neutral" | "published-market-neutral";
...
type RiskTab = "Factors"|"Positions"|"Correlations"|"Covariance Forecast";

function RiskWorkspace({ tab, model, total, holdings, analytics, payload, forecast, onSelectTicker }: { tab:RiskTab; model:FactorModelAnalytics|null; total:number; holdings:Holding[]; analytics:PortfolioAnalytics; payload:MarketPayload; forecast:CovarianceForecastAnalytics|null; onSelectTicker:(ticker:string)=>void }) {return <>{tab==="Factors"&&model&&<FactorRisk model={model} total={total}/>} {tab==="Positions"&&<PositionRisk holdings={holdings} total={total} analytics={analytics} onSelectTicker={onSelectTicker}/>} {tab==="Correlations"&&<CorrelationAnalysis payload={payload} holdings={holdings} startDate={analytics.startDate} endDate={analytics.endDate}/>} {tab==="Covariance Forecast"&&<CovarianceForecastView forecast={forecast}/>}</>}
...
function StrategyAnalysis({ strategy, snapshot, isUpdating, total, frequency, setFrequency, targetVolatility, setTargetVolatility, targetBeta, setTargetBeta, maxPosition, setMaxPosition }: { strategy:StrategyBacktestAnalytics|null; snapshot:RiskTargetSnapshot|null; isUpdating:boolean; total:number; frequency:StrategyRebalanceFrequency; setFrequency:(value:StrategyRebalanceFrequency)=>void; targetVolatility:number; setTargetVolatility:(value:number)=>void; targetBeta:number; setTargetBeta:(value:number)=>void; maxPosition:number; setMaxPosition:(value:number)=>void }) {
  ...
  const allocationScale=Math.max(maxPosition,...snapshot.weights.flatMap(item=>[item.currentWeight,item.targetWeight]),.01);
  return <section className="strategy-layout">
    <article className="panel strategy-controls"><PanelTitle title="Covariance-driven allocation" meta="CM-IEWMA forecast → constrained risk allocation → cash scaling"/><div className="strategy-control-body"><label><span>REBALANCE</span><div className="segmented"><button className={frequency==="Weekly"?"on":""} onClick={()=>setFrequency("Weekly")}>Weekly</button><button className={frequency==="Monthly"?"on":""} onClick={()=>setFrequency("Monthly")}>Monthly</button></div></label><label className="strategy-slider"><span><b>Annual risk ceiling</b><strong>{(targetVolatility*100).toFixed(0)}%</strong></span><input aria-label="Annual risk ceiling" type="range" min="8" max="25" step="1" value={targetVolatility*100} onChange={event=>setTargetVolatility(Number(event.target.value)/100)}/></label><label className="strategy-slider"><span><b>S&amp;P 500 beta ceiling</b><strong>{targetBeta.toFixed(2)}</strong></span><input aria-label="S&amp;P 500 beta ceiling" type="range" min="0.10" max="1.30" step="0.05" value={targetBeta} onChange={event=>setTargetBeta(Number(event.target.value))}/></label><label className="strategy-slider"><span><b>Maximum position</b><strong>{(maxPosition*100).toFixed(0)}%</strong></span><input aria-label="Maximum position" type="range" min="1" max="35" step="1" value={maxPosition*100} onChange={event=>setMaxPosition(Number(event.target.value)/100)}/></label></div><div className="strategy-control-effects"><div><b>Rebalance</b><span>Reruns the same point-in-time rule weekly or monthly. It does not alter the latest target shown beside the controls.</span></div><div><b>Risk ceiling</b><span>Caps forecast annual volatility. If it binds, the risky sleeve is scaled down and the remainder moves to cash.</span></div><div><b>Beta ceiling</b><span>Caps forecast market beta. It scales the covariance-derived risky sleeve; it is not an expected-return or timing signal.</span></div><div><b>Position ceiling</b><span>Caps every stock. Under this construction it can reduce the whole risky sleeve rather than silently redistribute risk.</span></div></div><div className="strategy-assumptions"><span><b>CM-IEWMA</b> covariance forecast</span><span><b>Constrained risk allocation</b> convex direction</span><span><b>Long-only + cash</b> no leverage</span><span><b>Next session</b> implementation</span></div></article>
    <article className={`panel strategy-live-allocation ${latestBinding?"binding":""}`} aria-live="polite"><PanelTitle title="Latest target allocation" meta={`${snapshot.asOf} → ${snapshot.effective.toLowerCase()}`}/><div className="live-allocation-summary"><div><span>FORECAST RISK</span><strong>{(snapshot.estimatedVolatility*100).toFixed(1)}%</strong><small>ceiling {(targetVolatility*100).toFixed(1)}%</small></div><div><span>ESTIMATED BETA</span><strong>{snapshot.beta.toFixed(2)}</strong><small>ceiling {targetBeta.toFixed(2)}</small></div><div><span>CASH</span><strong>{(snapshot.cashWeight*100).toFixed(1)}%</strong><small>{(snapshot.turnover*100).toFixed(1)}% one-way turnover</small></div><div><span>BINDING LIMIT</span><strong className="binding-label">{snapshot.bindingConstraints.join(" + ")}</strong><small>{snapshot.solverStatus}</small></div></div><div className="live-allocation-head"><span>POSITION</span><span>WEIGHT BAR</span><span>CURRENT</span><span>TARGET</span><span>CHANGE</span><span>RISK: BUDGET → FORECAST</span></div><div className="live-allocation-rows">{snapshot.weights.map(item=><div className="live-allocation-row" key={item.ticker}><strong>{item.ticker}</strong><div className="allocation-change-track"><i style={{width:`${item.currentWeight/allocationScale*100}%`}}/><b style={{width:`${item.targetWeight/allocationScale*100}%`}}/></div><span>{(item.currentWeight*100).toFixed(1)}%</span><span>{(item.targetWeight*100).toFixed(1)}%</span><em className={item.change<0?"negative":"positive"}>{item.change>=0?"+":""}{(item.change*100).toFixed(1)} pp</em><span className="risk-share">{(item.riskBudget*100).toFixed(1)}% → {(item.riskContribution*100).toFixed(1)}%</span></div>)}<div className="live-allocation-row cash"><strong>CASH</strong><div className="allocation-change-track"><i style={{width:`${Math.max(0,snapshot.currentCashWeight)/Math.max(allocationScale,snapshot.currentCashWeight,snapshot.cashWeight,.01)*100}%`}}/><b style={{width:`${Math.max(0,snapshot.cashWeight)/Math.max(allocationScale,snapshot.currentCashWeight,snapshot.cashWeight,.01)*100}%`}}/></div><span>{(snapshot.currentCashWeight*100).toFixed(1)}%</span><span>{(snapshot.cashWeight*100).toFixed(1)}%</span><em className={snapshot.cashWeight-snapshot.currentCashWeight<0?"negative":"positive"}>{snapshot.cashWeight-snapshot.currentCashWeight>=0?"+":""}{((snapshot.cashWeight-snapshot.currentCashWeight)*100).toFixed(1)} pp</em><span className="risk-share">—</span></div></div><p>The current risky holdings define fixed risk budgets. CM-IEWMA determines the relative stock weights that meet those budgets; the tightest displayed ceiling sets their common scale and cash receives the residual. Every number above is the direct output of that rule.</p></article>
    <div className={`strategy-recompute-state ${isUpdating?"updating":""}`}><b>{isUpdating?"Updating historical backtest…":"Historical backtest is current"}</b><span>The allocation above is the latest next-session target. The results below rerun the same rule independently at every historical rebalance.</span></div>
    <section className="metrics strategy-metrics"><Metric label="STRATEGY RETURN" value={pct(strategy.totalReturn*100)} sub={`${money(total*strategy.totalReturn)} after estimated costs`} tone="cyan"/><Metric label="REALIZED VOLATILITY" value={`${(strategy.annualVolatility*100).toFixed(1)}%`} sub={`Ex-ante ceiling ${(targetVolatility*100).toFixed(0)}%`} tone="amber"/><Metric label="REALIZED BETA" value={strategy.beta.toFixed(2)} sub={`Ex-ante ceiling ${targetBeta.toFixed(2)}`}/><Metric label="SHARPE RATIO" value={strategy.sharpeRatio.toFixed(2)} sub="Annualized · 0% risk-free rate" tone="cyan"/><Metric label="MAX DRAWDOWN" value={pct(strategy.maxDrawdown*100)} sub={`${strategy.startDate} — ${strategy.endDate}`} tone="red"/><Metric label="TOTAL TURNOVER" value={`${(strategy.totalTurnover*100).toFixed(0)}%`} sub={`${money(strategy.transactionCosts)} estimated cost`}/></section>
    <article className="panel strategy-sharpe"><PanelTitle title="Risk-adjusted performance" meta="Annualized Sharpe · same window · 0% risk-free rate"/><div className="strategy-sharpe-grid"><div className="primary"><span>COVARIANCE-CONTROLLED STRATEGY</span><strong>{strategy.sharpeRatio.toFixed(2)}</strong><em>Net of estimated trading costs</em></div><div><span>STATIC CURRENT PORTFOLIO</span><strong>{strategy.staticSharpeRatio.toFixed(2)}</strong><em className={sharpeVsStatic>=0?"positive":"negative"}>{sharpeVsStatic>=0?"+":""}{sharpeVsStatic.toFixed(2)} strategy difference</em></div><div><span>S&amp;P 500</span><strong>{strategy.benchmarkSharpeRatio.toFixed(2)}</strong><em className={sharpeVsBenchmark>=0?"positive":"negative"}>{sharpeVsBenchmark>=0?"+":""}{sharpeVsBenchmark.toFixed(2)} strategy difference</em></div></div><p>Sharpe is an ex-post performance diagnostic for the allocation sequence, not an objective of CM-IEWMA or the convex allocator. Cash is assumed to earn 0%, and every comparison uses the same historical window.</p></article>
    <article className="panel strategy-performance"><PanelTitle title="Point-in-time strategy backtest" meta={`${frequency.toLowerCase()} rebalancing · net of estimated costs`}/><PerformanceChart portfolio={strategy.curve} benchmark={strategy.staticCurve} tertiary={strategy.benchmarkCurve} dates={strategy.dates} startDate={strategy.startDate} endDate={strategy.endDate} large primaryLabel="Covariance-controlled strategy" secondaryLabel="Static current portfolio" tertiaryLabel="S&P 500" definition="Growth of $1 using only information available before each rebalance. Constrained-risk-allocation weights become effective on the following session; positions drift between rebalances. The static comparison applies today’s portfolio weights across the same period."/></article>
    ...
    <article className="panel strategy-rebalances"><PanelTitle title="Recent rebalance decisions" meta={`${strategy.rebalances.length} total rebalances · open a row for exact weights and risk shares`}/><div className="strategy-log-head"><span>DATE</span><span>EST. RISK</span><span>BETA</span><span>INVESTED</span><span>TURNOVER</span><span>BINDING / LARGEST</span></div>{recentRebalances.map(item=><details className="strategy-rebalance-detail" key={item.date}><summary className="strategy-log-row"><b>{item.date}</b><span>{(item.estimatedVolatility*100).toFixed(1)}%</span><span>{item.beta.toFixed(2)}</span><span>{(item.investedWeight*100).toFixed(1)}%</span><span>{(item.turnover*100).toFixed(1)}%</span><strong>{item.bindingConstraints.join(" + ")}<em>{item.largestPosition} {(item.largestWeight*100).toFixed(1)}%</em></strong></summary><div className="rebalance-weight-grid">{item.weights.map(weight=><div key={weight.ticker}><strong>{weight.ticker}</strong><i><b style={{width:`${Math.min(100,weight.weight/Math.max(maxPosition,.01)*100)}%`}}/></i><span>{(weight.weight*100).toFixed(2)}% · R {(weight.riskContribution*100).toFixed(1)}%</span></div>)}<div className="cash"><strong>CASH</strong><span>{Math.max(0,(1-item.investedWeight)*100).toFixed(2)}%</span></div></div></details>)}</article>
    ...
    <article className="panel strategy-method"><PanelTitle title="What the allocator solves" meta="Covariance only · no expected-return forecast"/><ol><li><b>Forecast covariance</b><span>CM-IEWMA combines five point-in-time covariance experts using their preceding predictive likelihood.</span></li><li><b>Set risk budgets</b><span>The starting risky portfolio’s capital proportions become fixed risk budgets ρ for the whole historical test.</span></li><li><b>Solve the convex direction</b><span>At each rebalance the allocator minimizes ½x′Σx − Σρᵢ log xᵢ. This makes each stock’s forecast risk contribution match its budget.</span></li><li><b>Scale and account</b><span>The largest common scale satisfying risk, beta, position and budget ceilings is applied next session. Cash absorbs the remainder; returns include 10 bps per unit of turnover.</span></li></ol><p className="method-source">The covariance forecast follows Johansson et al., <a href="https://github.com/cvxgrp/cov_pred_finance" target="_blank" rel="noreferrer">A Simple Method for Predicting Covariance Matrices of Financial Returns</a>. The allocation rule follows Johansson, <em>Convex Optimization for Quantitative Finance</em> (2025), chapter 6.</p></article>
  </section>;
}
...
export default function Home() {
  ...
  const [riskTab,setRiskTab]=useState<RiskTab>("Factors");
  ...
  const deferredStrategyFrequency=useDeferredValue(strategyFrequency);
  const deferredStrategyTargetVolatility=useDeferredValue(strategyTargetVolatility);
  const deferredStrategyTargetBeta=useDeferredValue(strategyTargetBeta);
  const deferredStrategyMaxPosition=useDeferredValue(strategyMaxPosition);
  const covarianceForecast=useMemo(()=>marketData&&!isPublishedFund?buildCovarianceForecast(marketData,holdings,analysisEnd||undefined):null,[marketData,holdings,analysisEnd,isPublishedFund]);
  const riskTargetSnapshot=useMemo(()=>marketData&&covarianceForecast&&!hasShort&&!isPublishedFund?buildRiskTargetSnapshot(marketData,holdings,covarianceForecast,{targetVolatility:strategyTargetVolatility,targetBeta:strategyTargetBeta,maxPosition:strategyMaxPosition}):null,[marketData,holdings,covarianceForecast,hasShort,isPublishedFund,strategyTargetVolatility,strategyTargetBeta,strategyMaxPosition]);
  const strategyAnalytics = useMemo(() => marketData && !hasShort && !isPublishedFund ? buildRiskTargetStrategy(marketData, holdings, { frequency: deferredStrategyFrequency, targetVolatility: deferredStrategyTargetVolatility, targetBeta: deferredStrategyTargetBeta, maxPosition: deferredStrategyMaxPosition, startDate: analysisStart || undefined, endDate: analysisEnd || undefined, lookback: 126, transactionCostBps: 10 }) : null, [marketData, holdings, hasShort, isPublishedFund, deferredStrategyFrequency, deferredStrategyTargetVolatility, deferredStrategyTargetBeta, deferredStrategyMaxPosition, analysisStart, analysisEnd]);
  const strategyBacktestUpdating=deferredStrategyFrequency!==strategyFrequency||deferredStrategyTargetVolatility!==strategyTargetVolatility||deferredStrategyTargetBeta!==strategyTargetBeta||deferredStrategyMaxPosition!==strategyMaxPosition;
  ...
  function loadDemo(key:DemoKey){const next=key==="published-market-neutral"?publishedMarketNeutralSeed:key==="market-neutral"?marketNeutralSeed:equitySeed;setDemoKey(key);setHoldings(next.map(item=>({...item})));setBaselineHoldings(next.map(item=>({...item})));setPortfolioSource("demo");setPortfolioName(key==="published-market-neutral"?"AQR Equity Market Neutral":key==="market-neutral"?"Long–Short Illustration":"US Equity Portfolio");setPortfolioError(null);setSelectedTicker(null);setEditing(false)}
  ...
  const viewCopy: Record<View, [string, string]> = {
    ...
    "Strategy Analysis": ["Strategy analysis", "Use the covariance forecast to construct a point-in-time portfolio under explicit risk ceilings."],
  ...
  const isCovarianceView=view==="Risk Analysis"&&riskTab==="Covariance Forecast";
  const displayedViewCopy:[string,string]=isCovarianceView?["Covariance forecast","Inspect the next-session CM-IEWMA covariance forecast used by portfolio risk and Strategy Lab."]:viewCopy[view];
    ...
    <main className="app-shell">
      ...
      <aside className="sidebar">
        <section className="portfolio-card">
          ...
          <label className="portfolio-switcher"><span>DEMO</span><select value={portfolioSource==="demo"?demoKey:"uploaded"} onChange={event=>event.target.value!=="uploaded"&&loadDemo(event.target.value as DemoKey)}><option value="equity">US Equity Portfolio</option><option value="published-market-neutral">AQR Market Neutral (QMNIX)</option><option value="market-neutral">Long–Short Illustration</option>{portfolioSource==="uploaded"&&<option value="uploaded">Uploaded portfolio</option>}</select></label>
      ...
      <section className="workspace">
        ...
        <div className="page-head">
          <div><span className="kicker">{view === "Overview" ? "LIVE PORTFOLIO INTELLIGENCE" : `FACTORSCOPE / ${primarySection.toUpperCase()}`}</span><h1>{displayedViewCopy[0]}</h1><p>{displayedViewCopy[1]}</p></div>
          {isCovarianceView?<div className="date-control covariance-date-control"><span>FORECAST AS OF</span><label><small>DATE</small><input type="date" aria-label="Covariance forecast date" min={availableStartDate} max={availableEndDate} value={analysisEnd} disabled={!availableEndDate} onChange={(event)=>setAnalysisEnd(event.target.value)} /></label></div>:view !== "Project Brief" && !(view === "Performance" && performanceTab === "Event Analysis") && <div className="date-control"><span>ANALYSIS PERIOD</span><label><small>FROM</small><input type="date" aria-label="Analysis start date" min={availableStartDate} max={analysisEnd || availableEndDate} value={analysisStart} disabled={!availableStartDate} onChange={(event)=>setAnalysisStart(event.target.value)} /></label><i>→</i><label><small>TO</small><input type="date" aria-label="Analysis end date" min={analysisStart || availableStartDate} max={availableEndDate} value={analysisEnd} disabled={!availableEndDate} onChange={(event)=>setAnalysisEnd(event.target.value)} /></label></div>}
        </div>
        ...
        {isPublishedFund && <PublishedFundContext />}
        {portfolioSource==="demo"&&demoKey==="market-neutral"&&<aside className="published-fund-context illustration-note"><strong>Long–short illustration · fixed starting positions</strong><p>Ten hand-selected pairs held through the replay, before the separately estimated costs. There is no stock-selection signal or rebalancing to maintain beta neutrality. Use AQR Market Neutral for an actual managed fund history.</p></aside>}
        {loading && !analytics && <DataLoading />}
        {!loading && !analytics && !dataError && <DataUnavailable />}
        {!loading && analytics && (!factorModel || !baselineFactorModel) && ((view === "Risk Analysis"&&riskTab!=="Covariance Forecast") || view === "Performance" || view === "Compare") && <FactorModelUnavailable />}
        {view === "Overview" && analytics && <Overview total={total} invested={invested} holdings={holdings} analytics={analytics} decomposition={portfolioDecomposition} onSelectTicker={setSelectedTicker} executionCostBps={executionCostBps} shortBorrowRate={shortBorrowRate} publishedFund={isPublishedFund} />}
        ...
        {view === "Strategy Analysis" && marketData && (isPublishedFund?<section className="data-state"><div><strong>The fund’s allocation is managed by AQR</strong><p>Daily underlying weights are not available here. Strategy Lab sliders cannot change the fund’s historical trades. Choose the US Equity Portfolio to construct and inspect a dynamic allocation.</p></div></section>:hasShort?<section className="data-state"><div><strong>Dynamic Strategy is currently long-only</strong><p>Switch to the US Equity Portfolio demo or remove short positions. The long–short illustration remains fully available in Portfolio, Risk &amp; Attribution, Compare, Performance and Scenarios.</p></div></section>:<StrategyAnalysis strategy={strategyAnalytics} snapshot={riskTargetSnapshot} isUpdating={strategyBacktestUpdating} total={total} frequency={strategyFrequency} setFrequency={setStrategyFrequency} targetVolatility={strategyTargetVolatility} setTargetVolatility={setStrategyTargetVolatility} targetBeta={strategyTargetBeta} setTargetBeta={setStrategyTargetBeta} maxPosition={strategyMaxPosition} setMaxPosition={setStrategyMaxPosition} />)}
        ...
        {view === "Risk Analysis" && analytics && marketData && !(isPublishedFund&&(riskTab==="Covariance Forecast"||riskTab==="Correlations")) && <RiskWorkspace tab={riskTab} model={factorModel} total={total} holdings={holdings} analytics={analytics} payload={marketData} forecast={covarianceForecast} onSelectTicker={setSelectedTicker} />}
        {view === "Risk Analysis" && isPublishedFund && (riskTab==="Covariance Forecast"||riskTab==="Correlations") && <section className="data-state"><div><strong>Underlying holdings history is unavailable</strong><p>The published demo provides the fund’s return series. A cross-security covariance or correlation matrix requires the underlying security universe and dated positions. Use the US Equity Portfolio or an uploaded portfolio for these tools.</p></div></section>}
...
function Overview({ total, invested, holdings, analytics, decomposition, onSelectTicker, executionCostBps, shortBorrowRate, publishedFund = false }: { total: number; invested: number; holdings: Holding[]; analytics: PortfolioAnalytics; decomposition: SecurityDrilldownAnalytics["decomposition"] | null; onSelectTicker: (ticker: string) => void; executionCostBps:number; shortBorrowRate:number; publishedFund?:boolean }) {
  ...
  return <>
    ...
    {decomposition && <article className="panel overview-decomposition"><PanelTitle title="Return anatomy" meta="Common factors + idiosyncratic return = observed portfolio return"/><div className="decomposition-answer-strip"><div><span>OBSERVED RETURN</span><strong>{pct(observedReturn*100)}</strong><small>What the portfolio delivered</small></div><div><span>COMMON-FACTOR RETURN</span><strong>{pct(systematicReturn*100)}</strong><small>Market, style and sector effects</small></div><div className="idio-answer"><span>IDIOSYNCRATIC RETURN</span><strong>{pct(idiosyncraticReturn*100)}</strong><small>{publishedFund?"Unexplained by the US proxy model":"The remainder beyond common factors"}</small></div></div><DecompositionChart decomposition={decomposition}/></article>}
...
function ProjectBrief({ setView }:{ setView:(view:View)=>void }) { return <section className="brief-grid">
  ...
  <article className="panel brief-thesis"><PanelTitle title="Why decomposition matters" meta="Exposure is not the same as differentiated return"/><div><span>COMMON FACTORS</span><strong>Accessible exposures</strong><p>If most performance is explained by known factors, the portfolio’s return and Sharpe will be driven toward the behavior of those common premia.</p></div><div><span>IDIOSYNCRATIC REMAINDER</span><strong>Signal plus noise</strong><p>Residual return is not automatically skill. The relevant evidence is persistence, factor-adjusted Sharpe and out-of-sample consistency.</p></div><div><span>MOCK STRATEGY</span><strong>Test decisions through time</strong><p>Rebalance weekly or monthly using CM-IEWMA and constrained risk allocation, then compare Sharpe, turnover, cash, realized risk and drawdown with the static portfolio and market benchmark without using future data.</p></div></article>
