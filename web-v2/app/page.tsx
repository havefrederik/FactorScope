...
function CovarianceForecastView({ forecast }: { forecast: CovarianceForecastAnalytics | null }) {
  const [matrixMode,setMatrixMode]=useState<"Correlation"|"Covariance">("Correlation");
  if(!forecast)return <section className="data-state"><div><strong>Covariance forecast unavailable</strong><p>CM-IEWMA needs at least 500 aligned daily return observations for every priced position in the portfolio.</p></div></section>;
  const matrix=matrixMode==="Correlation"?forecast.correlationMatrix:forecast.covarianceMatrix;
  const matrixScale=Math.max(...matrix.flat().map(value=>Math.abs(value)),1e-12);
  const dominant=forecast.expertWeights.reduce((best,expert)=>expert.weight>best.weight?expert:best,forecast.expertWeights[0]);
  const maxVolatility=Math.max(...forecast.securities.map(item=>item.volatility),.01);
  const cellStyle=(value:number)=>{const intensity=Math.min(1,Math.abs(value)/(matrixMode==="Correlation"?1:matrixScale));return {background:value<0?`rgba(255,92,112,${.08+intensity*.48})`:`rgba(37,210,198,${.07+intensity*.48})`}};
  return <section className="covariance-layout">
    <section className="metrics covariance-metrics"><Metric label="FORECAST HORIZON" value="Next session" sub={`Data through ${forecast.asOf}`} tone="cyan"/><Metric label="PORTFOLIO VOLATILITY" value={`${(forecast.portfolioVolatility*100).toFixed(1)}%`} sub="Annualized · current weights" tone="amber"/><Metric label="AVERAGE CORRELATION" value={forecast.averageCorrelation.toFixed(2)} sub={forecast.strongestPair?`Strongest: ${forecast.strongestPair.tickerA} / ${forecast.strongestPair.tickerB}`:"Cross-security average"}/><Metric label="DIVERSIFICATION RATIO" value={`${forecast.diversificationRatio.toFixed(2)}×`} sub="Weighted standalone risk / portfolio risk"/><Metric label="DOMINANT EXPERT" value={dominant.label} sub={`${(dominant.weight*100).toFixed(1)}% likelihood weight`} tone="cyan"/></section>
    <article className="panel covariance-matrix-panel"><PanelTitle title="CM-IEWMA covariance forecast" meta={`${forecast.tickers.length} securities · ${forecast.observations} aligned returns`}/><div className="covariance-toolbar"><p>This is the next-session forecast used by Strategy Lab. Correlation is the normalized view; covariance is annualized return² × 10,000.</p><div className="segmented"><button className={matrixMode==="Correlation"?"on":""} onClick={()=>setMatrixMode("Correlation")}>Correlation</button><button className={matrixMode==="Covariance"?"on":""} onClick={()=>setMatrixMode("Covariance")}>Covariance</button></div></div><div className="covariance-matrix-scroll"><table aria-label={`${matrixMode} forecast matrix`}><thead><tr><th>SECURITY</th>{forecast.tickers.map(ticker=><th key={ticker}>{ticker}</th>)}</tr></thead><tbody>{forecast.tickers.map((ticker,row)=><tr key={ticker}><th>{ticker}</th>{matrix[row].map((value,column)=><td key={`${ticker}-${forecast.tickers[column]}`} style={cellStyle(value)} title={`${ticker} / ${forecast.tickers[column]}: ${matrixMode==="Correlation"?value.toFixed(4):(value*10000).toFixed(4)}`}>{matrixMode==="Correlation"?value.toFixed(2):(value*10000).toFixed(1)}</td>)}</tr>)}</tbody></table></div><p className="covariance-definition">Estimated after the close on {forecast.asOf} using only returns available through that date. The matrix forecasts the covariance of the following trading session; it does not forecast expected returns.</p></article>
    <article className="panel covariance-experts"><PanelTitle title="Expert combination" meta="Trailing 10-session Gaussian likelihood"/><div className="covariance-expert-list">{forecast.expertWeights.map(expert=><div key={expert.label}><span>{expert.label}</span><i><b style={{width:`${expert.weight*100}%`}}/></i><strong>{(expert.weight*100).toFixed(1)}%</strong></div>)}</div><p>The five experts use the published volatility/correlation half-life pairs. Their precision factors are combined using the displayed simplex weights.</p></article>
    <article className="panel covariance-securities"><PanelTitle title="Security volatility forecast" meta="Annualized diagonal of the covariance matrix"/><div className="covariance-security-head"><span>SECURITY</span><span>PORTFOLIO WEIGHT</span><span>FORECAST RANGE</span><span>FORECAST VOL.</span><span>SHARE OF RISK</span></div>{forecast.securities.map(item=><div className="covariance-security-row" key={item.ticker}><strong>{item.ticker}</strong><span>{(item.portfolioWeight*100).toFixed(1)}%</span><div><i style={{width:`${item.volatility/maxVolatility*100}%`}}/></div><b>{(item.volatility*100).toFixed(1)}%</b><em className={item.riskShare<0?"negative":""}>{(item.riskShare*100).toFixed(1)}%</em></div>)}</article>
    <article className="panel covariance-method"><PanelTitle title="How to read this forecast" meta="One model output · three portfolio uses"/><ol><li><b>Risk</b><span>Current portfolio weights convert the covariance matrix into forecast portfolio volatility.</span></li><li><b>Allocation</b><span>Strategy Lab uses the same matrix when searching for weights near the selected risk and beta targets.</span></li><li><b>Diversification</b><span>Off-diagonal terms capture whether securities are expected to move together, not just their standalone volatility.</span></li></ol><p className="method-source">CM-IEWMA implementation follows Johansson, Ogut, Pelger, Schmelzer and Boyd, <a href="https://github.com/cvxgrp/cov_pred_finance" target="_blank" rel="noreferrer">A Simple Method for Predicting Covariance Matrices of Financial Returns</a>.</p></article>
  </section>;
}
...
export default function Home() {
  ...
  const covarianceForecast=useMemo(()=>marketData?buildCovarianceForecast(marketData,holdings,analysisEnd||undefined):null,[marketData,holdings,analysisEnd]);
  const riskTargetSnapshot=useMemo(()=>marketData&&covarianceForecast&&!hasShort?buildRiskTargetSnapshot(marketData,holdings,covarianceForecast,{targetVolatility:strategyTargetVolatility,targetBeta:strategyTargetBeta,maxPosition:strategyMaxPosition}):null,[marketData,holdings,covarianceForecast,hasShort,strategyTargetVolatility,strategyTargetBeta,strategyMaxPosition]);
  const strategyAnalytics = useMemo(() => marketData && !hasShort ? buildRiskTargetStrategy(marketData, holdings, { frequency: deferredStrategyFrequency, targetVolatility: deferredStrategyTargetVolatility, targetBeta: deferredStrategyTargetBeta, maxPosition: deferredStrategyMaxPosition, startDate: analysisStart || undefined, endDate: analysisEnd || undefined, lookback: 126, transactionCostBps: 10 }) : null, [marketData, holdings, hasShort, deferredStrategyFrequency, deferredStrategyTargetVolatility, deferredStrategyTargetBeta, deferredStrategyMaxPosition, analysisStart, analysisEnd]);
  ...
  return (
    <main className="app-shell">
      ...
      <section className="workspace">
        ...
        {view === "Risk Analysis" && analytics && marketData && <RiskWorkspace tab={riskTab} model={factorModel} total={total} holdings={holdings} analytics={analytics} payload={marketData} forecast={covarianceForecast} onSelectTicker={setSelectedTicker} />}
