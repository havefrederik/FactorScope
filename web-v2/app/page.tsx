...
function StrategyAnalysis({ strategy, total, frequency, setFrequency, targetVolatility, setTargetVolatility, targetBeta, setTargetBeta, maxPosition, setMaxPosition }: { strategy:StrategyBacktestAnalytics|null; total:number; frequency:StrategyRebalanceFrequency; setFrequency:(value:StrategyRebalanceFrequency)=>void; targetVolatility:number; setTargetVolatility:(value:number)=>void; targetBeta:number; setTargetBeta:(value:number)=>void; maxPosition:number; setMaxPosition:(value:number)=>void }) {
  ...
  return <section className="strategy-layout">
    ...
    <article className="panel strategy-exposures"><PanelTitle title="Risk exposure through time" meta="Ex-ante estimates at each trading day"/><StrategyExposureChart strategy={strategy} targetVolatility={targetVolatility} targetBeta={targetBeta}/><p>These are trailing model estimates, not realized outcomes. Exposure changes after each rebalance and then drifts with security returns until the next rebalance.</p></article>
    <article className="panel strategy-rebalances"><PanelTitle title="Recent rebalance decisions" meta={`${strategy.rebalances.length} total rebalances · newest first`}/><div className="strategy-log-head"><span>DATE</span><span>EST. RISK</span><span>BETA</span><span>INVESTED</span><span>TURNOVER</span><span>LARGEST</span></div>{recentRebalances.map(item=><div className="strategy-log-row" key={item.date}><b>{item.date}</b><span>{(item.estimatedVolatility*100).toFixed(1)}%</span><span>{item.beta.toFixed(2)}</span><span>{(item.investedWeight*100).toFixed(1)}%</span><span>{(item.turnover*100).toFixed(1)}%</span><strong>{item.largestPosition}<em>{(item.largestWeight*100).toFixed(1)}%</em></strong></div>)}</article>
    <article className="panel strategy-allocation"><PanelTitle title="Ending strategy allocation" meta={`Average invested ${(strategy.averageInvestedWeight*100).toFixed(1)}%`}/>{strategy.finalWeights.slice(0,8).map(item=><div className="strategy-weight-row" key={item.ticker}><strong>{item.ticker}</strong><div><i style={{width:`${Math.min(100,item.weight/Math.max(maxPosition,.01)*100)}%`}}/></div><span>{(item.weight*100).toFixed(1)}%</span></div>)}<div className="strategy-cash-row"><strong>CASH</strong><span>{Math.max(0,(1-strategy.finalWeights.reduce((sum,item)=>sum+item.weight,0))*100).toFixed(1)}%</span></div></article>
    <article className="panel strategy-method"><PanelTitle title="What this test does" meta="Transparent standard configuration"/><ol><li><b>Estimate</b><span>Trailing covariance and market betas use the preceding 126 trading days only.</span></li><li><b>Construct</b><span>A long-only portfolio seeks the requested volatility and beta while limiting turnover and concentration.</span></li><li><b>Rebalance</b><span>Target weights are implemented the next session; unused capital remains in cash.</span></li><li><b>Evaluate</b><span>Returns are shown after a 10 bps turnover cost. This is a historical simulation, not a forecast.</span></li></ol></article>
  </section>;
}
...
export default function Home() {
  ...
  const [baselineHoldings, setBaselineHoldings] = useState(seed);
  ...
  const strategyAnalytics = useMemo(() => marketData ? buildRiskTargetStrategy(marketData, holdings, { frequency: strategyFrequency, targetVolatility: strategyTargetVolatility, targetBeta: strategyTargetBeta, maxPosition: strategyMaxPosition, startDate: analysisStart || undefined, endDate: analysisEnd || undefined, lookback: 126, transactionCostBps: 10 }) : null, [marketData, holdings, strategyFrequency, strategyTargetVolatility, strategyTargetBeta, strategyMaxPosition, analysisStart, analysisEnd]);
  ...
  function applySecurityValue(items: Holding[], ticker: string, requestedValue: number) {
    ...
    const targetValue=Math.max(0,Number.isFinite(requestedValue)?requestedValue:0);
  ...
  function updateWeight(ticker: string, percent: number) { setHoldings(items => {
    ...
    const targetWeight=Math.max(0,Number.isFinite(percent)?percent:0)/100;
    return applySecurityValue(items,ticker,portfolioTotal*targetWeight);
  }); }
  function normalizeInvested(){setHoldings(items=>{const portfolioTotal=items.reduce((sum,item)=>sum+item.value,0);const investedTotal=items.filter(item=>item.ticker!=="CASH").reduce((sum,item)=>sum+item.value,0);if(portfolioTotal<=0||investedTotal<=0)return items;const scale=portfolioTotal/investedTotal;return items.map(item=>item.ticker==="CASH"?{...item,value:0}:{...item,value:item.value*scale})})}
  ...
  return (
    <main className="app-shell">
      ...
      <section className="workspace">
        ...
        {view === "Overview" && analytics && <Overview total={total} invested={invested} holdings={holdings} analytics={analytics} decomposition={portfolioDecomposition} onSelectTicker={setSelectedTicker} />}
...
function PortfolioComparison({ holdings, baselineHoldings, total, baselineTotal, analytics, baselineAnalytics, model, baselineModel, onSaveBaseline }: { holdings: Holding[]; baselineHoldings: Holding[]; total: number; baselineTotal: number; analytics: PortfolioAnalytics; baselineAnalytics: PortfolioAnalytics; model: FactorModelAnalytics; baselineModel: FactorModelAnalytics; onSaveBaseline: () => void }) {
  ...
  return <section className="comparison-layout">
    ...
    <article className="panel comparison-factors">
      ...
      <div className="comparison-factor-head"><span>FACTOR</span><span>BASELINE</span><span>CURRENT</span><span>CHANGE</span></div>
      {factorRows.map((factor) => <div className="comparison-factor-row" key={factor.name}><div><strong>{factor.name}</strong><small>{factor.group}</small></div><span>{factor.baselineExposure>=0?"+":""}{factor.baselineExposure.toFixed(2)}</span><span>{factor.exposure>=0?"+":""}{factor.exposure.toFixed(2)}</span><div className="factor-change"><i className={factor.delta<0?"negative":""} style={{width:`${Math.abs(factor.delta)/largestFactorMove*48}%`}}/><strong className={factor.delta>0?"delta-positive":factor.delta<0?"delta-negative":""}>{factor.delta>=0?"+":""}{factor.delta.toFixed(2)}</strong></div></div>)}
    </article>
  </section>;
}

function Overview({ total, invested, holdings, analytics, decomposition, onSelectTicker }: { total: number; invested: number; holdings: Holding[]; analytics: PortfolioAnalytics; decomposition: SecurityDrilldownAnalytics["decomposition"] | null; onSelectTicker: (ticker: string) => void }) {
...
function FactorRisk({ model, total }: { model: FactorModelAnalytics; total: number }) {
  ...
  return <>
    ...
    <section className="factor-layout refined-factor-layout">
      ...
      <article className="panel risk-split"><PanelTitle title="Where variance comes from" meta="Final model R²" /><div className="split-number"><strong>{(model.rSquared*100).toFixed(0)}%</strong><span>Systematic</span><strong>{(idiosyncraticShare*100).toFixed(0)}%</strong><span>Idiosyncratic</span></div><div className="splitbar"><i style={{width:`${model.rSquared*100}%`}}/><b style={{width:`${idiosyncraticShare*100}%`}}/></div><p>Risk shares are covariance-based incremental estimates. Factor P&amp;L is arithmetically linked through time; the idiosyncratic remainder makes attribution reconcile exactly to observed portfolio P&amp;L.</p></article>
    </section>
  </>;
}
...
function ProjectBrief({ setView }:{ setView:(view:View)=>void }) { return <section className="brief-grid">
  ...
  <article className="panel brief-skills"><PanelTitle title="What this project demonstrates" meta="Research translated into product"/><div><span>QUANTITATIVE RESEARCH</span><p>Factor modeling, covariance risk, point-in-time portfolio construction, turnover and historical replay.</p></div><div><span>PRODUCT JUDGMENT</span><p>Clear scope, transparent assumptions and deliberate separation of exposure, risk and return.</p></div><div><span>IMPLEMENTATION</span><p>Responsive interface, editable state, CSV ingestion and interactive strategy and scenario controls.</p></div></article>
</section> }
