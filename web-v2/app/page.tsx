...
function CovarianceModelCard({ result, selected }: { result: StrategyBacktestAnalytics; selected: boolean }) {
  return <div className={selected?"selected":""}><header><span>{result.covarianceModel}</span>{selected&&<em>ACTIVE</em>}<strong>{result.covarianceModelLabel}</strong></header><dl><div><dt>AVG. PREDICTED RISK</dt><dd>{(result.averageEstimatedVolatility*100).toFixed(1)}%</dd></div><div><dt>REALIZED RISK</dt><dd>{(result.annualVolatility*100).toFixed(1)}%</dd></div><div><dt>CALIBRATION GAP</dt><dd>{result.volatilityCalibrationGap>=0?"+":""}{(result.volatilityCalibrationGap*100).toFixed(1)} pp</dd></div><div><dt>NET SHARPE</dt><dd>{result.sharpeRatio.toFixed(2)}</dd></div><div><dt>RETURN</dt><dd>{pct(result.totalReturn*100)}</dd></div><div><dt>TURNOVER</dt><dd>{(result.totalTurnover*100).toFixed(0)}%</dd></div></dl>{result.covarianceModel==="CM-IEWMA"&&<div className="expert-mix"><b>AVERAGE EXPERT MIX</b>{result.covarianceExpertWeights.map(expert=><span key={expert.label}><em>{expert.label}</em><i><strong style={{width:`${expert.weight*100}%`}}/></i><small>{(expert.weight*100).toFixed(0)}%</small></span>)}</div>}</div>;
}

function StrategyAnalysis({ strategy, covarianceComparison, covarianceModel, setCovarianceModel, total, frequency, setFrequency, targetVolatility, setTargetVolatility, targetBeta, setTargetBeta, maxPosition, setMaxPosition }: { strategy:StrategyBacktestAnalytics|null; covarianceComparison:StrategyBacktestAnalytics|null; covarianceModel:CovarianceModel; setCovarianceModel:(value:CovarianceModel)=>void; total:number; frequency:StrategyRebalanceFrequency; setFrequency:(value:StrategyRebalanceFrequency)=>void; targetVolatility:number; setTargetVolatility:(value:number)=>void; targetBeta:number; setTargetBeta:(value:number)=>void; maxPosition:number; setMaxPosition:(value:number)=>void }) {
  ...
  return <section className="strategy-layout">
    ...
    {covarianceComparison&&<article className="panel strategy-model-comparison"><PanelTitle title="Covariance forecast comparison" meta="Same universe, targets, dates, rebalance schedule and costs"/><div className="covariance-model-grid"><CovarianceModelCard result={strategy} selected/><CovarianceModelCard result={covarianceComparison} selected={false}/></div><p>CM-IEWMA combines five volatility/correlation forecasts using their likelihood over the preceding 10 sessions. This is a risk-model comparison: any return or Sharpe difference comes from the allocations each covariance forecast produced, not from a return forecast.</p></article>}
    ...
    <article className="panel strategy-method"><PanelTitle title="What this test does" meta="Transparent standard configuration"/><ol><li><b>Forecast risk</b><span>{strategy.covarianceModel==="CM-IEWMA"?"Five IEWMA experts are combined from recent predictive likelihood; the fastest expert receives 5% diagonal regularization.":"The sample covariance uses the preceding 126 sessions."}</span></li><li><b>Estimate beta</b><span>Market betas use the preceding 126 sessions independently of the covariance-model choice.</span></li><li><b>Construct</b><span>A long-only portfolio seeks the requested volatility and beta while limiting turnover and concentration; weights apply next session.</span></li><li><b>Evaluate</b><span>Returns are shown after a 10 bps turnover cost. The covariance is ex-ante; the strategy result is a historical simulation.</span></li></ol><p className="method-source">Method adapted from Johansson, Ogut, Pelger, Schmelzer and Boyd, <a href="https://github.com/cvxgrp/cov_pred_finance" target="_blank" rel="noreferrer">A Simple Method for Predicting Covariance Matrices of Financial Returns</a>.</p></article>
  </section>;
}
...
export default function Home() {
  ...
  const [strategyCovarianceModel,setStrategyCovarianceModel]=useState<CovarianceModel>("CM-IEWMA");
  ...
  const strategyAnalytics = useMemo(() => marketData && !hasShort ? buildRiskTargetStrategy(marketData, holdings, { frequency: strategyFrequency, targetVolatility: strategyTargetVolatility, targetBeta: strategyTargetBeta, maxPosition: strategyMaxPosition, startDate: analysisStart || undefined, endDate: analysisEnd || undefined, lookback: 126, transactionCostBps: 10, covarianceModel: strategyCovarianceModel }) : null, [marketData, holdings, hasShort, strategyFrequency, strategyTargetVolatility, strategyTargetBeta, strategyMaxPosition, strategyCovarianceModel, analysisStart, analysisEnd]);
  const strategyCovarianceComparison = useMemo(() => marketData && !hasShort ? buildRiskTargetStrategy(marketData, holdings, { frequency: strategyFrequency, targetVolatility: strategyTargetVolatility, targetBeta: strategyTargetBeta, maxPosition: strategyMaxPosition, startDate: analysisStart || undefined, endDate: analysisEnd || undefined, lookback: 126, transactionCostBps: 10, covarianceModel: strategyCovarianceModel === "CM-IEWMA" ? "Rolling 126-day" : "CM-IEWMA" }) : null, [marketData, holdings, hasShort, strategyFrequency, strategyTargetVolatility, strategyTargetBeta, strategyMaxPosition, strategyCovarianceModel, analysisStart, analysisEnd]);
