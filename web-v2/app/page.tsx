...
function StrategyAnalysis({ strategy, snapshot, isUpdating, total, frequency, setFrequency, targetVolatility, setTargetVolatility, targetBeta, setTargetBeta, maxPosition, setMaxPosition }: { strategy:StrategyBacktestAnalytics|null; snapshot:RiskTargetSnapshot|null; isUpdating:boolean; total:number; frequency:StrategyRebalanceFrequency; setFrequency:(value:StrategyRebalanceFrequency)=>void; targetVolatility:number; setTargetVolatility:(value:number)=>void; targetBeta:number; setTargetBeta:(value:number)=>void; maxPosition:number; setMaxPosition:(value:number)=>void }) {
  ...
  const latestBinding=Math.abs(snapshot.estimatedVolatility-targetVolatility)>.018||Math.abs(snapshot.beta-targetBeta)>.12;
  ...
  return <section className="strategy-layout">
    <article className="panel strategy-controls"><PanelTitle title="Set the allocation rule" meta="Sliders change the latest target; rebalance changes the historical schedule"/><div className="strategy-control-body"><label><span>REBALANCE</span><div className="segmented"><button className={frequency==="Weekly"?"on":""} onClick={()=>setFrequency("Weekly")}>Weekly</button><button className={frequency==="Monthly"?"on":""} onClick={()=>setFrequency("Monthly")}>Monthly</button></div></label><label className="strategy-slider"><span><b>Annual risk target</b><strong>{(targetVolatility*100).toFixed(0)}%</strong></span><input aria-label="Annual risk target" type="range" min="8" max="25" step="1" value={targetVolatility*100} onChange={event=>setTargetVolatility(Number(event.target.value)/100)}/></label><label className="strategy-slider"><span><b>S&amp;P 500 beta target</b><strong>{targetBeta.toFixed(2)}</strong></span><input aria-label="S&amp;P 500 beta target" type="range" min="0.10" max="1.30" step="0.05" value={targetBeta} onChange={event=>setTargetBeta(Number(event.target.value))}/></label><label className="strategy-slider"><span><b>Maximum position</b><strong>{(maxPosition*100).toFixed(0)}%</strong></span><input aria-label="Maximum position" type="range" min="1" max="35" step="1" value={maxPosition*100} onChange={event=>setMaxPosition(Number(event.target.value)/100)}/></label></div><div className="strategy-control-effects"><div><b>Rebalance</b><span>Changes how often the rule is rerun in the historical backtest. It does not change the latest point-in-time target shown beside the controls.</span></div><div><b>Risk target</b><span>Changes total risky exposure. A low target can move capital to cash when the stock mix cannot be made quiet enough.</span></div><div><b>Beta target</b><span>Tilts the stock mix toward lower- or higher-beta names while the covariance forecast controls total risk.</span></div><div><b>Maximum position</b><span>Hard-caps every stock. Lower caps spread the allocation and can make the requested targets infeasible.</span></div></div><div className="strategy-assumptions"><span><b>CM-IEWMA</b> covariance forecast</span><span><b>126 days</b> market beta</span><span><b>Long-only</b> no leverage</span><span><b>Next session</b> target implementation</span></div></article>
    <article className={`panel strategy-live-allocation ${latestBinding?"binding":""}`} aria-live="polite"><PanelTitle title="Latest target allocation" meta={`${snapshot.asOf} → ${snapshot.effective.toLowerCase()}`}/><div className="live-allocation-summary"><div><span>FORECAST RISK</span><strong>{(snapshot.estimatedVolatility*100).toFixed(1)}%</strong><small>requested {(targetVolatility*100).toFixed(1)}%</small></div><div><span>ESTIMATED BETA</span><strong>{snapshot.beta.toFixed(2)}</strong><small>requested {targetBeta.toFixed(2)}</small></div><div><span>CASH</span><strong>{(snapshot.cashWeight*100).toFixed(1)}%</strong><small>{(snapshot.turnover*100).toFixed(1)}% turnover</small></div></div><div className="live-allocation-head"><span>POSITION</span><span>WEIGHT BAR</span><span>CURRENT</span><span>TARGET</span><span>CHANGE</span></div><div className="live-allocation-rows">{snapshot.weights.map(item=><div className="live-allocation-row" key={item.ticker}><strong>{item.ticker}</strong><div className="allocation-change-track"><i style={{width:`${item.currentWeight/allocationScale*100}%`}}/><b style={{width:`${item.targetWeight/allocationScale*100}%`}}/></div><span>{(item.currentWeight*100).toFixed(1)}%</span><span>{(item.targetWeight*100).toFixed(1)}%</span><em className={item.change<0?"negative":"positive"}>{item.change>=0?"+":""}{(item.change*100).toFixed(1)} pp</em></div>)}<div className="live-allocation-row cash"><strong>CASH</strong><div className="allocation-change-track"><i style={{width:`${Math.max(0,snapshot.currentCashWeight)/Math.max(allocationScale,snapshot.currentCashWeight,snapshot.cashWeight,.01)*100}%`}}/><b style={{width:`${Math.max(0,snapshot.cashWeight)/Math.max(allocationScale,snapshot.currentCashWeight,snapshot.cashWeight,.01)*100}%`}}/></div><span>{(snapshot.currentCashWeight*100).toFixed(1)}%</span><span>{(snapshot.cashWeight*100).toFixed(1)}%</span><em className={snapshot.cashWeight-snapshot.currentCashWeight<0?"negative":"positive"}>{snapshot.cashWeight-snapshot.currentCashWeight>=0?"+":""}{((snapshot.cashWeight-snapshot.currentCashWeight)*100).toFixed(1)} pp</em></div></div><p>{latestBinding?"The requested combination is not attainable at the latest rebalance under the displayed position cap, long-only and no-leverage constraints. These are the closest feasible weights.":"These are the exact next-session target weights implied by the displayed controls and latest point-in-time estimates."}</p></article>
